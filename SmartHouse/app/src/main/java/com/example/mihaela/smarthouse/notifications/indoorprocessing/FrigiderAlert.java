 ...
 * @author RGI3
 */
public class FrigiderAlert implements Alert{
    @Override
    public void JsonAnalyser(JSONArray jsonArray, String TargetURL) {
        ...
        for(int i=0;i<jsonArray.length();i++){
            try {
                ...
                message=null;
                if(Integer.parseInt(temperatura)>5){
                    ASmartUnit smUnit = new SmartFridge(id, nume);
                    smUnit.resetToDefault(id);
                    String alertMessage = "The temperature in the fridge was above 5 degrees Celsius. The values have been reset.";
                ...
                } else if(Integer.parseInt(temperatura)<-10){
                    ASmartUnit smUnit = new SmartFridge(id, nume);
                    smUnit.resetToDefault(id);
                    String alertMessage = "The temperature in the fridge was below -10 degrees Celsius. The values have been reset.";
                    NotificationsActivity.newNotifications.add(new NotificationsListItem("Alert", alertMessage));
                    AlertsManager.displayAlert(alertMessage);
                }
